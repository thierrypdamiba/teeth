# momentum-strict
Momentum-only: if the last three closes all move the same direction I back the move at p=0.53 (shrunk from 0.55); mixed bars → p=0.50. Revision policy: Reconsider and shrink further when the 12-run mean Brier > 0.255 or after 4 consecutive incorrect momentum calls.
